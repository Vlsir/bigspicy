...
class SpiceWriter():
  ...
  def WriteModule(self, module, file_name, append=False):
  ...
  def WriteAll(self, file_name):
    with open(file_name, 'w') as f:
      for name, path_and_module in (
          self.design.modules_by_name_then_path.items()):
        for path, module in path_and_module.items():
          print(f'writing {name} (path: {path}) to {file_name}')
          self.WriteModule(module, file_name, append=True)
